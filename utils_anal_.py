...
def plot_training_curve(stat_dir, loss_only=False):
    try:
        f=open(stat_dir, 'rb')
...
def load_net(epoch=None, save_=None): #load lastest model saved in save_dir_stat
    if epoch is None:
        net_path = get_last_model(model_prefix=model_name + "_epoch_", base_dir=dir_, is_dir=True)
    else:
...
def plot_ratio(data, data2, logger, name="undefined", name2="undefined_sum", save_dir=None):
    ensure_dir(save_dir)
    if(data2 is not None):
        ...
        for i in range(input_num):
            try:
                ...
                norm_ratio[i]=0.0
        plot_dist(data=norm_ratio, logger=None, name=name+" norm ratio", save_dir=save_dir, bins=30, sci_float=False)
        '''
        ...
        plot_dist(data=ratio, logger=logger, name=name+" ratio", save_dir=save_dir)
        '''
    else:
        ...
        for i in range(data_num):
            try:
                plot_dist(data=data_ratio[i], logger=logger, name=name[i]+" ratio in "+name2, save_dir=save_dir, bins=30, sci_float=False)
            except Exception:
                logger.write("exception in dist_plot:"+str(name))
                traceback.print_exc()
                input()

def plot_dist(data, logger, name="undefined", ran=[], save_dir=None, bins=100, kde=False, hist=True, notes="", redo=False, 
...
def plot_log_dist(data, logger, name="undefined", save_dir=None, cumulative=False, hist=True, kde=False, bins=60):
    to_np_array(data)
    data=data.flatten()
    data_log=[]
    count_0=0
...
def visualize_res_weight_corr(net, res_data, save_dir):
    ensure_dir(save_dir)
    ...
    print(r_corr.shape)
    imsave(save_dir + "/" + "N-N correlation.png", r_corr)

def visualize_weight(net, name="r", save_dir=None):
    ensure_dir(save_dir)
    color_map = plt.cm.get_cmap('jet')
    ...
    w = color_map(w)
    w = np.uint8(w * 255)
    imsave(save_dir + "/" + "N-N connection.png", w)    

def plot_rf(net, save_dir=None, name="undefined", sample_num=16):#RF:(neuron_num, input_num)
    ...
    for range_ in ranges:
        ...
        imsave(save_dir + "/" + "rf_%s.png"%(names[count]), imgs_cat)
        count += 1
...
def plot_res(data, logger=None, name="undefined", save_dir=None, is_act=True, sample_num=3):
    ensure_dir(save_dir)
    ...
    if(is_act==True):
        ...
        plot_dist(data=act_ratio, logger=logger, name=name+" single neuron active ratio", hist=True, kde=False, save_dir=save_dir, bins=60)
        plot_dist(data=input_act_ratio, logger=logger, name=name+" neurons active ratio to single input", hist=True, kde=False, save_dir=save_dir, bins=60)
        plot_dist(data=neuron_act_ratio, logger=logger, name=name+" single neuron active ratio to inputs", hist=True, kde=False, save_dir=save_dir, bins=60, cumulative=True) 
        plot_log_dist(data=data.flatten(), logger=logger, name=name, hist=True, kde=False, save_dir=save_dir, bins=60)

    plot_dist(data=data.flatten(), logger=logger, name=name, hist=True, kde=False, save_dir=save_dir, bins=60)   

    #plot single neuron response to images
    for i in range(sample_num):
        unit_index=random.randint(0,neuron_num-1)
        plot_dist(data=data[unit_index], logger=logger, name="response of %s unit No.%d(%d)"%(name,unit_index,neuron_num-1), save_dir=save_dir + "single neuron responses/", bins=100)

    #plot neurons response to single image
    for i in range(sample_num):
        input_index=random.randint(0,input_num-1)
        plot_dist(data=data[:, input_index], logger=logger, name="%s response to input No.%d(%d)"%(name,input_index,input_num-1), save_dir=save_dir + "single input responses/", bins=100)

def plot_corr(x, y, name=["data0", "data1"], save_dir=None, anal_stat=True, density_plot=False, log_plot=True, nonlog_plot=True):
    ensure_dir(save_dir)
    #print(name)
    #print(x)
    #print(y)
    ...
    if nonlog_plot:
        ...
        if density_plot: #takes much time
            ...
            plt.savefig(save_dir+title+"(density).jpg")
            plt.cla()
            plt.close("all")
    ...
    if log_plot:
        ...
        plt.title(title, fontsize=large_fontsize)
        plt.savefig(save_dir+title+"(log).jpg")
        plt.tight_layout(pad=0.4, w_pad=2.0, h_pad=2.0)
        plt.cla()
        plt.close("all")
        ...
        if density_plot: #takes much time
            ...
            sns.jointplot(x=name[0], y=name[1], data=df, kind="kde");
            plt.savefig(save_dir+title+"(density)(log).jpg")
            plt.cla()
            plt.close("all")
...
def anal_stability(net, iter_data, save_dir=None, input_sample_num=5, neuron_sample_num=20):
    ensure_dir(save_dir)
    if net.dict["separate_ei"]:
    ...
    for key in keys:
        ...
        for input_index in random.sample(range(data.size(0)), input_sample_num):
            ...
            plt.savefig(save_dir + key + "/" + "sampled %d neuron responses to image No.%d"%(neuron_sample_num, input_index) + ".jpg")
            plt.close()
...
def get_corr(x, y, method="all", save=False, name="undefined", save_dir=None):
    corr={}
...
def cal_pearson_corr(data, data_2=None, name="unnamed_corr", method="pearson", save=False, save_dir=None):
...
def cal_corr(data, data_2=None, name="unnamed_corr", method="pearson", save=False, save_dir=None):#data:(unit_num, feature_vector_size)
...
def plot_weight_stat(weight, logger, name="undefined", save_dir=None):
    ensure_dir(save_dir)

    is_positive = plot_log_dist(weight, logger, name=name, save_dir=save_dir)
    plot_log_dist(weight, logger, name=name, save_dir=save_dir, cumulative=True)
    ...
    plot_dist(data=weight_flat, logger=logger, name=name, save_dir=save_dir, notes=note0)
    plot_dist(data=weight_flat, logger=logger, name=name, save_dir=save_dir, cumulative=True, notes=note0)
...
def plot_weight_rec(data, name="undefined", save_dir=None, density_plot=False, log_plot=True):
    ensure_dir(save_dir)
    unit_num = data.shape[0]
    data_num = unit_num ** 2
...
def plot_weight_lat(data, data2, name="undefined", save_dir=None, density_plot=False, log_plot=True):
    ensure_dir(save_dir)
