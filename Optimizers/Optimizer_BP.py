...
class Optimizer_BP(Optimizer):
    ...
    def train(self, x, y):
        ...
        if isinstance(perform, dict):
            ...
            raise Exception('Invalid perform type: %s'%type(perform))
        #loss = results["loss"]
        loss.backward()
        self.optimizer.step()
...
class Optimizer_BP(Optimizer):
    def __init__(self, dict_=None, load=False, options=None):
        super().__init__(dict_, load=False)
        if self.scheduler is not None:
            self.dict["scheduler_dict"] = None
        else:
            self.dict["scheduler_dict"] = self.scheduler.state_dict()
        self.optimizer = build_optimizer_(self.dict["optimizer"], self.model, load=load)

    def train(self, x, y, iter_time=None):
        self.optimizer.zero_grad()
        
        results = self.model.get_loss(input_data)
        loss = results["loss"]
        loss.backward()
        self.optimizer.step()
'''